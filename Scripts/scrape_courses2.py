...
def get_course_listings(soup):
  ...
  return [{'deptANDnum': match.group('dept')+" "+match.group('num')} for match in LISTING_REGEX.finditer(listings)]
...
def get_single_class(row):
  ...
  if bldg_link.string.strip() == "Frist Campus Center":
    return {
      ...
      'roomnum': "frist"+bldg_link.nextSibling.string.replace('&nbsp;', ' ').strip(),
    }
...
#       cells = row('td')
#       bldg_link = cells[4].strong.a
#       building = bldg_link.string.strip()
#       if building == "Frist Campus Center":
